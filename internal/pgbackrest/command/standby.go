...
// ErrNoCurrentPrimary is returned when backup-from-standby is enabled but the
// cluster reports no current primary, which is not a healthy cluster state.
var ErrNoCurrentPrimary = errors.New("cluster has no current primary")

// ShouldConfigurePrimaryPeer reports whether a backup running on instanceName
// should coordinate with the primary. Whether a backup lands on the primary or a
// standby is decided by CloudNativePG via the backup target, not here.
func ShouldConfigurePrimaryPeer(enabled bool, currentPrimary, instanceName string) (bool, error) {
	if !enabled {
		return false, nil
	}
	if currentPrimary == "" {
		return false, ErrNoCurrentPrimary
	}
	return currentPrimary != instanceName, nil
}
